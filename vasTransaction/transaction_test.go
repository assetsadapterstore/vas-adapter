...
func Test_case1(t *testing.T) {
	// 前置交易ID和UTXO对应的索引
	in := Vin{"d134ceb5255b6e4901ce768ffa658807e4ebe5d4874d78e3e95497a27f539401", uint32(0)}
	// 目标地址和发送金额
	out := Vout{"VJzZnE5jLoUCoG58UR9PHx9ssKTQBAaRN7", uint64(900000)}
	...
	addressPrefix := AddressPrefix{[]byte{0x46}, []byte{0x05}, nil, "vas"}
...
func Test_case2(t *testing.T) {
	...
	addressPrefix := AddressPrefix{[]byte{0x6f}, []byte{0xc4}, nil, "tb"}
...
func Test_DER(t *testing.T) {

	// 前置交易ID和UTXO对应的索引
	in := Vin{"aebe5604298d0a2ac05891a6966e153ce916355264d98a90a651109cb5c62c39", uint32(0)} // mzsts8xiVWv8uGEYUrAB6XzKXZPiX9j6jq
	// 目标地址和发送金额
	out := Vout{"mirTWDJzHBKbJi3YfNjYdwnyk5EL1jZG1L", uint64(19990000)}
...
func Test_LTC_new_p2sh_as_output(t *testing.T) {
	// 前置交易ID和UTXO对应的索引
	in := Vin{"5a02061670d3fa5cdc63854f75c607db6ddc7818ceb637015e1bf8f22649a55b", uint32(0)}
	// 目标地址和发送金额
	out := Vout{"MGjXD6TT621ngpAsYGFom78WerEAZunARm", uint64(900000)}
	...
	///////构建空交易单
	emptyTrans, err := CreateEmptyRawTransaction([]Vin{in}, []Vout{out}, lockTime, replaceable, LTCMainnetAddressPrefix)
	...
	/////////计算待签名交易单哈希
	transHash, err := CreateRawTransactionHashForSig(emptyTrans, []TxUnlock{unlockData}, segwit, LTCMainnetAddressPrefix)
	if err != nil {
		t.Error("创建待签交易单哈希失败")
	} else {
		for i, t := range transHash {
	...
	// 验证交易单
	pass := VerifyRawTransaction(signedTrans, []TxUnlock{unlockData}, segwit, LTCMainnetAddressPrefix)
	if pass {
		fmt.Println("验证通过!")
	} else {
		t.Error("验证失败!")
...
func Test_LTC_bech32_as_output(t *testing.T) {
	// 前置交易ID和UTXO对应的索引
	in := Vin{"5a02061670d3fa5cdc63854f75c607db6ddc7818ceb637015e1bf8f22649a55b", uint32(0)}
	// 目标地址和发送金额
	out := Vout{"ltc1q5c9vun5ctq377nfkznaxlj7nh5a0esm90n89t7", uint64(900000)}
