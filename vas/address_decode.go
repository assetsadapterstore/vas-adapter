...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := vas_addrdec.VAS_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = vas_addrdec.VAS_testnetPrivateWIFCompressed
	}

	wif, _ := vas_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	vas_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := vas_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := vas_addrdec.VAS_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = vas_addrdec.VAS_testnetAddressP2SH
	}
	...
	address, _ := vas_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := vas_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
